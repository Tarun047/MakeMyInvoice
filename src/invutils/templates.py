...
class SimpleInvoice(SimpleDocTemplate):
    ...
    def __init__(self, invoice_path, pdf_info=None, precision='0.01',discount=0):
        if not pdf_info:
    ...
    def _service_provider_data(self):
        if isinstance(self.service_provider_info, ServiceProviderInfo):
            props = [('name', 'Name'), ('street', 'Street'), ('city', 'City'), ('state', 'State'),
                     ('country', 'Country'), ('post_code', 'Post code'), ('vat_tax_number', 'GST Number')]
    ...
    def _client_info_data(self):
        ...
        props = [('name', 'Name'), ('street', 'Street'), ('city', 'City'), ('state', 'State'),
                 ('country', 'Country'), ('post_code', 'Post code'), ('email', 'Email'), ('client_id', 'Client id'),('vat_tax_number', 'Party GST')]
    ...
    def _item_data_and_style(self):
        ...
        item_data_title = ('Name', 'Unit Price', 'Units', 'HSN','GST','Amount')
        ...
        roundtotal = self.getroundeddecimal(total*(100-self.discount)/100, self.precision)

        #EDIT
        item_data.append(('Discount', '', '', '','', '{} %'.format(self.discount)))
        sum_start_y_index += 1
        style.append(('SPAN', (0, sum_start_y_index), (sum_start_x_index, sum_start_y_index)))
        style.append(('ALIGN', (0, sum_start_y_index), (sum_end_x_index, -1), 'RIGHT'))
        #EDITEND
